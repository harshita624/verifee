...
import {
  ...
  CheckCircle, Navigation, ArrowRight, Info, ChevronDown,
} from "lucide-react";
...
function PriceResult({ result }) {
  ...
  const riskLabel  = overcharge>=60?"High tourist premium":overcharge>=30?"Moderate premium":"Fair pricing";
  ...
  return (
    <div className="space-y-4 animate-fade-up">
        ...
        <div className="px-6 py-5" style={{ background: "#16a34a" }}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <h2 className="text-[20px] font-black text-white">{result.product}</h2>
            ...
            <div className="text-right">
              ...
              <p className="text-[30px] font-black text-white">{result.confidenceScore}%</p>
        ...
        <div className="p-5 bg-white space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <p className="text-[32px] font-black text-zinc-950">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5">
            {[
              { label:"Local market avg",  value:formatPrice(result.localMarketAvg),   textColor:"#09090b" },
              { label:"Online avg",        value:formatPrice(result.onlineAvg),         textColor:"#09090b" },
              { label:"Start bargaining",  value:formatPrice(result.bargainingStart),   textColor:"#dc2626" },
              { label:"Target price",      value:formatPrice(result.bargainingTarget),  textColor:"#16a34a" },
            ].map(item => (
              <div key={item.label} className="rounded-xl p-3" style={{ background:"#fafafa" }}>
                <p className="text-[10px] text-zinc-400 mb-1">{item.label}</p>
                <p className="text-[15px] font-bold" style={{ color:item.textColor }}>{item.value}</p>
              </div>
            ))}
          </div>
      ...
      <div className="flex gap-2.5 flex-wrap">
        <Link href={`/scam-check?product=${encodeURIComponent(result.product)}&city=${encodeURIComponent(result.city)}`}
          ...
          <AlertTriangle className="w-4 h-4" /> Check if price is a scam
        </Link>
        <Link href={`/translate?text=${encodeURIComponent("What is your best price?")}`}
        ...
        <Link href={`/bargain?product=${encodeURIComponent(result.product)}&city=${encodeURIComponent(result.city)}`}
...
function CheckPriceContent() {
  ...
  const [cityInput, setCityInput] = useState(city||searchParams.get("city")||"");

  useEffect(() => { if (city) setCityInput(city); }, [city]);
  ...
  useEffect(() => {
    if (searchParams.get("q")) handleCheck(null, searchParams.get("q"), searchParams.get("city")||city);
  }, []);

  const handleCheck = async (e, overrideQ, overrideCity) => {
    e?.preventDefault();
    const q = overrideQ || query;
    const c = overrideCity !== undefined ? overrideCity : cityInput;
    if (!q.trim()) return;

    setLoading(true); setError(""); setResult(null);

    // Build product string with place context
    const productWithPlace = selectedPlace
      ? `${q.trim()} at ${selectedPlace}`
      : q.trim();
      ...
      <div className="py-10 border-b border-zinc-100 bg-white">
        <div className="container max-w-3xl">
          ...
          <p className="text-[14px] text-zinc-500 mb-7">
            ...
            {cityInput && <span className="font-semibold text-green-600"> Showing prices for {cityInput}.</span>}
          </p>
          ...
          <form onSubmit={handleCheck} className="space-y-3">
            ...
            <div className="flex items-center gap-2 bg-white rounded-xl border border-zinc-200 px-4 py-3 focus-within:border-green-400 transition-colors">
              ...
              <input
                ...
                placeholder="e.g. Kashmiri Carpet, Hotel room, Auto ride, Banarasi Saree"
            ...
            <div className="flex gap-3">
              <div className="flex items-center gap-2 flex-1 bg-white rounded-xl border border-zinc-200 px-4 py-2.5 focus-within:border-green-400 transition-colors">
                <MapPin className="w-4 h-4 text-zinc-400 shrink-0" />
                <input
                  type="text" value={cityInput}
                  onChange={e=>{ setCityInput(e.target.value); setManualCity(e.target.value); setSelectedPlace(""); }}
                  placeholder="City (auto-detected or type)"
                ...
                <button type="button" onClick={detectLocation} disabled={detecting}
                  title="Detect my city" className="shrink-0 p-1 rounded-lg hover:bg-green-50">
                  {detecting
            ...
            {category === "other" && (
              <input
                ...
                placeholder="Type your category e.g. Street food, Religious items, Sports equipment"
                className="w-full rounded-xl border border-zinc-200 px-4 py-2.5 text-[14px] text-zinc-700 bg-white focus:border-green-400"
              />
            )}

            {/* City places dropdown */}
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    ...
                    {places.map(p => (
                      <button
                        ...
                        title={`Tourist risk: ${p.touristRisk}`}
                      >
                        {p.name}
                        {p.touristRisk==="high" && (
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 text-white font-semibold text-[14px] py-3 rounded-xl transition-all active:scale-95 disabled:opacity-50"
              style={{ background: loading||!query.trim() ? "#86efac" : "#16a34a", cursor: !query.trim()?"not-allowed":"pointer" }}
            >
              {loading
          ...
          <div className="flex items-center gap-2 flex-wrap mt-4">
            ...
            {[
              ...
              { q:"Hotel room",    c:cityInput||"Goa",    p:"" },
              { q:"Kashmiri Carpet",c:"Srinagar", p:"Lal Chowk" },
              { q:"Auto ride",     c:cityInput||"Delhi",  p:"" },
            ].map(({q,c,p}) => (
              <button key={q} onClick={()=>{ setQuery(q); setCityInput(c); setManualCity(c); setSelectedPlace(p); }}
      ...
      <div className="container max-w-3xl py-8">
        {error && (
          <div className="rounded-2xl bg-red-50 border border-red-100 p-5 text-center mb-6">
            ...
            <p className="text-[14px] font-semibold text-red-700 mb-1">Could not get price estimate</p>
        ...
        {loading && (
          <div className="text-center py-16">
            ...
            <p className="text-[13px] text-zinc-400">
              Checking community data {selectedPlace?`at ${selectedPlace}`:""}
            </p>
          </div>
        )}
        ...
        {!loading && !result && !error && (
          <div className="text-center py-16">
            ...
            <p className="text-[15px] font-semibold text-zinc-700 mb-2">Search any product or service</p>
            <p className="text-[13px] text-zinc-400">From handicrafts to hotel rooms — we know what the fair price is.</p>
          </div>
        )}
      </div>
    </div>
